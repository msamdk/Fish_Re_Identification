...
# --- SCRIPT TO FINE-TUNE SWIN-T USING METRIC LEARNING (TRIPLET LOSS) ---
# --- This version has been adapted from the ResNet50 script. ---
...
# --- MODIFIED FOR SWIN-T ---
EXPERIMENT_NAME = "SwinT_exp_1_batch_16_hard_margin_0.5_euclidean"
...
BEST_MODEL_SAVE_PATH = os.path.join(MODEL_SAVE_DIR, "best_swinT_triplet_finetuned.pth")
...
# --- MODIFIED FOR SWIN-T ---
# Swin-T's output feature dimension before the classification head is 768
...
# --- MODIFIED Model Definition FOR SWIN-T ---
class FishReIDNet(nn.Module):
    def __init__(self, backbone_out_dim, embedding_dim, pretrained=True):
        super().__init__()
        print(f"Initializing FishReIDNet with Swin-T backbone (pretrained={pretrained})")
        
        # 1. Set the correct pretrained weights for Swin-T
        weights = Swin_T_Weights.IMAGENET1K_V1 if pretrained else None
        
        # 2. Instantiate the Swin-T model
        self.backbone = swin_t(weights=weights)
        
        # 3. Remove the final classification layer (for Swin-T, it's called 'head')
        self.backbone.head = nn.Identity()
        
        # The embedding head remains the same, but now takes the new backbone_out_dim
...
class FishCropDataset(Dataset):
        ...
        try:
            ...
            if self.is_train or (build_map_if_not_train_and_no_map and not id_to_label_map):
                ...
                print(f"  Warning: Validation dataset created without an initial id_to_label_map and 'build_map_if_not_train_and_no_map' is False. "
                      f"IDs not in the provided map (if any was given) will get label -1.")
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # Return a placeholder on image loading error
            return torch.zeros(3, IMG_SIZE, IMG_SIZE), torch.tensor(-1, dtype=torch.long)
...
# The following helper classes and functions (ProbabilisticAugmentor, PKsampler, ResizeAndPadToSquare)
# are model-agnostic and do not need changes. They are included here for completeness.

#--- custom class to perform augmentation only for a certain percentage of the time ---
...
class PKsampler(Sampler):
    def __init__(self, dataset, p, k):
        ...
        for idx, label_item in enumerate(all_labels):
            if label_item != -1:
                self.labels_to_indices[label_item].append(idx)
...
# The rest of the functions (train_one_epoch, validate_model, etc.) are also model-agnostic
# and are included here for completeness without changes.
...
def train_one_epoch(model, train_loader, optimizer, loss_fn, miner, device, epoch_num):
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        ...
        try:
            ...
            if "no triplets" in str(e).lower() or "at least 2 classes" in str(e).lower() or "must contain labels from at least 2 classes" in str(e).lower():
                ...
                print(f"Epoch {epoch_num} Training: Unexpected error in Batch {batch_idx}: {e}. Skipping batch.")
                skipped_batches_miner += 1
                continue
        if loss is not None and torch.is_tensor(loss) and loss.numel() == 1 and torch.isfinite(loss):
            ...
            if not np.isnan(current_loss_item) and not np.isinf(current_loss_item) and current_loss_item >= 0:
                ...
                avg_loss_display_str = f"{avg_loss_calc_val:.4f}" if not (np.isnan(avg_loss_calc_val) or np.isinf(avg_loss_calc_val)) else "N/A_avg"
    ...
    if num_valid_batches_processed > 0:
        ...
        print(f"Epoch {epoch_num} Training: No valid batches were processed.")
        return float('inf')
...
def validate_model(model, val_loader, device, accuracy_metrics_to_include, accuracy_k_value, loss_fn_val, miner_fn_val, epoch_num):
    ...
    all_val_embeddings_list, all_val_labels_list = [], []
    total_val_loss, num_val_loss_batches = 0.0, 0
    with torch.no_grad():
        for images, labels in tqdm(val_loader, desc=f"  Epoch {epoch_num} - Validating", leave=False, dynamic_ncols=True):
            ...
            images_gpu, labels_gpu = images_batch.to(device), labels_batch.to(device)
    ...
    avg_val_loss = total_val_loss / num_val_loss_batches if num_val_loss_batches > 0 else float('inf')
    if np.isnan(avg_val_loss) or np.isinf(avg_val_loss): avg_val_loss = float('nan')
    ...
    print(f"  Epoch {epoch_num} - Validation samples for metrics: {all_val_embeddings.shape[0]}")
    if torch.isnan(all_val_embeddings).any() or torch.isinf(all_val_embeddings).any():
        print(f"  Epoch {epoch_num} - FATAL: NaNs/Infs detected in validation embeddings.");
        return {'precision_at_1': 0.0, 'mean_average_precision_at_r': 0.0}, avg_val_loss
    if len(all_val_labels) < 2 or len(torch.unique(all_val_labels)) < 2:
        print(f"  Epoch {epoch_num} - Warning: Validation set too small for metric calculation.");
        return {'precision_at_1': 0.0, 'mean_average_precision_at_r': 0.0}, avg_val_loss
    accuracies = {}
    try:
        accuracy_calculator_cpu = AccuracyCalculator(include=accuracy_metrics_to_include, k=accuracy_k_value, device=torch.device("cpu"))
        accuracies = accuracy_calculator_cpu.get_accuracy(all_val_embeddings, all_val_labels, all_val_embeddings, all_val_labels)
    except Exception as e_cpu:
        print(f"  Epoch {epoch_num} - ERROR during CPU accuracy calculation: {e_cpu}\n{traceback.format_exc()}")
        accuracies = {'precision_at_1': 0.0, 'mean_average_precision_at_r': 0.0}
...
def extract_and_save_embeddings(model, dataloader, device, save_path, dataset_label_map):
    ...
    print(f"Extracting embeddings for saving to '{save_path}'...")
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Extracting Embeddings"):
            ...
            all_original_labels_list.append(labels_b.cpu().numpy())
...
def plot_training_curves(train_losses, val_losses, val_map_rs, num_epochs_completed, save_path):
    ...
    plt.title('Validation mAP@R Accuracy'); plt.xlabel('Epochs'); plt.ylabel('mAP@R Accuracy (%)')
    plt.legend(); plt.grid(True)
...
if __name__ == "__main__":
    ...
    # --- TRANSFORMATION PIPELINES (UPDATED FOR SWIN-T) ---
    print("\n--- Setting up Datasets and Loaders with Standard PyTorch Transforms ---")

    # Use the standard ImageNet normalization values that Swin-T was pretrained on.
    # This is critical for good performance when using a pretrained model.
    ...
    try:
        ...
        if len(train_sampler) == 0 and len(train_dataset) > 0:
            print("FATAL: PK Sampler created 0 batches. Check P, K, and data distribution."); exit()
    ...
    print("\n--- Training Finished ---")
    final_model_to_report = BEST_MODEL_SAVE_PATH if best_epoch != -1 and os.path.exists(BEST_MODEL_SAVE_PATH) else os.path.join(MODEL_SAVE_DIR, f"model_epoch_{NUM_EPOCHS}.pth")
    ...
    if os.path.exists(final_model_to_report) and val_loader is not None and len(val_loader.dataset) > 0:
        print(f"\n--- Extracting Embeddings using model: {os.path.basename(final_model_to_report)} ---")
        extraction_model = FishReIDNet(BACKBONE_OUTPUT_DIM, EMBEDDING_DIM, pretrained=False)
        extraction_model.to(DEVICE)
        try:
            extraction_model.load_state_dict(torch.load(final_model_to_report, map_location=DEVICE))
        ...
        except Exception as e: print(f"Error during embedding extraction: {e}\n{traceback.format_exc()}")

    print(f"\nTotal script run time: {time.time() - main_start_time:.2f}s ({(time.time() - main_start_time)/60:.2f}m)")
    print(f"\n*** Use saved model for EVALUATION script ***")
