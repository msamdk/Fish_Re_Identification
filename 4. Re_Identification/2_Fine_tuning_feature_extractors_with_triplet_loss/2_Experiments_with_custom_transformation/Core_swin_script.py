...
# Using ImageNet stats as per your configuration
...
class ResizeAndPadToSquare:
    ...
    def __call__(self, img):
        ...
        if original_w > original_h: new_w, new_h = self.output_size, int(self.output_size * (original_h / original_w))
        elif original_h > original_w: new_h, new_w = self.output_size, int(self.output_size * (original_w / original_h))
        else: new_w = self.output_size; new_h = self.output_size
        resized_img = TF.resize(img, (new_h, new_w), interpolation=TF.InterpolationMode.BICUBIC)
        pad_left, pad_top = (self.output_size - new_w) // 2, (self.output_size - new_h) // 2
        pad_right, pad_bottom = self.output_size - new_w - pad_left, self.output_size - new_h - pad_top
        padding = (pad_left, pad_top, pad_right, pad_bottom)
        return TF.pad(resized_img, padding, fill=self.fill_color, padding_mode='constant')
